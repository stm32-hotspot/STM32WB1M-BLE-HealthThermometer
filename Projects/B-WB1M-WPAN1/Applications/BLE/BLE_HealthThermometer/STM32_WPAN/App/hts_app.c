...
/* USER CODE BEGIN HTS_App_Notification */
  switch(pNotification->HTS_Evt_Opcode)
  {
    ...
    case HTS_INTERMEDIATE_TEMPERATURE_NOT_ENABLED_EVT:
      {
        APP_DBG_MSG("HTS_INTERMEDIATE_TEMPERATURE_NOTIF_ENABLED_EVT\n");
    ...
    case HTS_INTERMEDIATE_TEMPERATURE_NOT_DISABLED_EVT:
      {
        APP_DBG_MSG("HTS_INTERMEDIATE_TEMPERATURE_NOTIF_DISABLED_EVT\n");
...
/* USER CODE BEGIN HTSAPP_Init */
  ...
  HTSAPP_Context.TemperatureMeasurementChar.TimeStamp.Year = 2023;
  HTSAPP_Context.TemperatureMeasurementChar.TimeStamp.Month = 10;
  HTSAPP_Context.TemperatureMeasurementChar.TimeStamp.Day = 1;
  HTSAPP_Context.TemperatureMeasurementChar.TimeStamp.Hours = 8;
  HTSAPP_Context.TemperatureMeasurementChar.TimeStamp.Minutes = 30;
...
/* USER CODE BEGIN HTSAPP_Measurement */
  ...
  {
    measurement = ieee11073_float_to_uint32_t(ENV_Get_Temperature());
  }  
#else
  measurement = ieee11073_float_to_uint32_t(ENV_Get_Temperature());
#endif
...
/* USER CODE BEGIN HTSAPP_IntermediateTemperature */
  ...
  uint32_t measurement;
  float temp_f = ENV_Get_Temperature();
  measurement = ieee11073_float_to_uint32_t(temp_f);
...
/* USER CODE BEGIN FD */
uint32_t ieee11073_float_to_uint32_t (float temperature_f)
{
  uint8_t exponent = 0xFE; 
  uint32_t mantissa = (uint32_t)(temperature_f * 100);
